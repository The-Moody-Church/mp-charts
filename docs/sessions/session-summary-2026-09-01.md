...
2. **PR 2 `salvage/auth-secret-fail-fast`** (this branch): BETTER_AUTH_SECRET fail-fast at module load with `NEXT_PHASE === "phase-production-build"` guard (covers Docker builder + CI's bare `npm run build`; better-auth 1.6.26's own production throw is a first-request rejected promise, and non-production containers silently accept its default secret — this fails at boot instead). Finding #18: `getUserInfo` extracted to exported `getMpUserInfo(accessToken)`; `sanitizeGuid(profile.sub)` hoisted out of the enrichment try — a malformed `sub` now fails the login, and the validated GUID becomes `id` and `userGuid`. Finding #19: `checkRateLimit` denies blank/whitespace userId with `retryAfterMs = windowMs`. Tests: +2 fail-fast (dynamic import under stubbed env), +4 getMpUserInfo (malformed sub rejected pre-MP-lookup, validated GUID round-trip, enrichment-failure tolerance, non-OK userinfo), +3 rate-limit blank-id. Audit report landed with superseded header, per-finding fixed statuses, and three editor's-note corrections.
3. **PR 3 `upstream/adopt-77-75-better-auth-pin`** (this branch): #77 — `client-credentials.ts` + rewritten `client.test.ts` clean-checked-out from upstream `664adff` (both byte-identical bases verified), `client.ts` hand-ported (3 constants + `expires_in` clamp; upstream's `console.log` not reintroduced; `isUserToken` branch untouched), fork-only user-token test added; mutation check: flat-cap restore fails exactly the 4 lifecycle tests. #75 — 9 guards swapped to `sanitizeId()` (8 planned + `journey-tools getJourneyMilestones`; summer-blast's per-item guard deliberately kept — it accumulates failures instead of throwing and its service boundary re-validates), service-boundary `sanitizeId` in `updateContactLog`/`deleteContactLog`, `Number.isSafeInteger` in `sanitizeId` with 3 boundary tests + injection tests incl. new `contact-lookup-details/actions.test.ts` (5 tests; each action's throw-vs-fallback contract asserted). better-auth pinned `>=1.6.23 <1.7.0` (lockfile synced, installed 1.6.26 unchanged). Sync-log `## Review: 2026-09-01` section (12 verdicts), 3 ideas.md carry-forwards, security.md action-boundary bullet.
4. **PR 4 `chore/deps-2026-09-next-16.3.3`** (this branch): lockfile-only `npm update` — next landed at 16.3.4 (newer than the 16.3.3 target, within `^16.3.0`), tsx 4.23.13, react-hook-form 7.87.0, @hookform/resolvers 5.9.1, eslint-config-next 16.3.4; vitest already at range-max. `package.json` untouched. Verified: `npm ci`, lint, 664 tests, build, `npm audit` 0 vulnerabilities.
