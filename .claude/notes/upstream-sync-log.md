...
| #85 | Contact-log attribution server-authoritative (F4) | **Deferred** to its own PR | Applies partially: our `Made_By` is protected only by **spread order** in the action, and a smuggled `Contact_ID` on update can re-parent a log onto another contact. Will be **adapted**, not copied — upstream stamps the *editor* because any role-holder may edit any log; our policy is owner-only edit with the original author preserved, so we strip in the service and never send `Made_By` on update |
